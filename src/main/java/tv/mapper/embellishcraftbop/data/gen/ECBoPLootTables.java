...
public class ECBoPLootTables extends ECLootTables
{
    ...
    protected void addTables()
    {
        for(int j = 0; j < Arrays.stream(BoPWoods.values()).count(); j++)
        {
            ...
            lootTables.put(ECBoPBlocks.BOP_LARGE_SUSPENDED_STAIRS_BLOCKS.get(BoPWoods.byId(j)).get(),
                    createStandardTable(EmbellishCraftBOP.MODID, ECBoPBlocks.BOP_LARGE_SUSPENDED_STAIRS_BLOCKS.get(BoPWoods.byId(j)).get()));
            lootTables.put(ECBoPBlocks.BOP_WOODEN_CRATE_BLOCKS.get(BoPWoods.byId(j)).get(),
                createCrateTable(EmbellishCraftBOP.MODID, ECBoPBlocks.BOP_WOODEN_CRATE_BLOCKS.get(BoPWoods.byId(j)).get()));
            lootTables.put(ECBoPBlocks.BOP_FANCY_CHEST_BLOCKS.get(BoPWoods.byId(j)).get(),
                createChestTable(BoPWoods.byId(j).getString() + "_fancy_chest", ECBoPBlocks.BOP_FANCY_CHEST_BLOCKS.get(BoPWoods.byId(j)).get()));
        }
